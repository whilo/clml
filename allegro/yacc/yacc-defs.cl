...
(defparameter *yacc-version* "YACC Release 0.98")

(pushnew (intern *yacc-version* :keyword) *features*)
...
#+sbcl
(defmethod clos:validate-superclass ((class grammar-class)
...
(defclass grammar ()
  ((lexer :accessor grammar-lexer	;A function returning terminal tokens.  A lexer
   ...
   (sp          :accessor grammar-sp          :initform nil) ;Stack pointer
   ;;A guess how big the stack should be.
   (initial-stack-size :accessor grammar-initial-stack-size :initform 61)
...
(defmethod shared-initialize :after ((grammar grammar-class) slots &rest initargs)
  ...
  (unless (composedp grammar)
    (let ((terminals
      ...
      (when (member 'eof terminals)	;rfe5995
